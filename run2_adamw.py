"""
WasteNet AI - Run 2: Optimization & Regularization Training Script
Author: Yahav
Date: May 2026

Final Metrics (Run 2):
- Train Loss: 0.1050 | Train Acc: 96.18%
- Val Loss:   0.5652 | Val Acc:   87.97%
- Test Acc:   87.99%

Description: Improved baseline using AdamW optimizer, weight decay, and basic data augmentations (RandomResizedCrop, Flip, Rotate).
"""
import os
import shutil
import torch
...
BACKUP_DIR = "./checkpoints/Run2"
os.makedirs(BACKUP_DIR, exist_ok=True)
writer = SummaryWriter(f'runs/{run_name}')

hparams = {
    'img_size': 224, 'batch_size': 32, 'epochs': 80,
    'optimizer': 'AdamW', 'learning_rate': 0.001, 'weight_decay': 0.01, 'augmentations': 'RandomResizedCrop, Flip, Rotate'
}

normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
train_transforms = transforms.Compose([transforms.RandomResizedCrop(hparams['img_size'], scale=(0.7, 1.0)), transforms.RandomHorizontalFlip(p=0.5), transforms.RandomRotation(15), transforms.ToTensor(), normalize])
